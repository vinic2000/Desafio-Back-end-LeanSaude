...
export class TransferService {
  ...
  async transfer(dataTransfer: TransferDto) {
    ...
    let payeeAccount = await this.prisma.account.findUnique({
      where: {
        userId: payee,
      },
    });
    ...
    if (!payeeAccount) {
      ...
      throw new HttpException("Payee account don't find", HttpStatus.NOT_FOUND);
    }

    let payerAccount = await this.prisma.account.findUnique({
      where: {
        userId: payer,
      },
    });
    ...
    if (!payerAccount) {
      ...
      throw new HttpException("Payer account don't find", HttpStatus.NOT_FOUND);
    }

    const { type_user } = await this.prisma.user.findUnique({
      where: {
        id: payer,
      },
    });
